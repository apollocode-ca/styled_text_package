...
class TextSpanEditingController extends TextEditingController {
  TextSpanEditingController({required TextSpan textSpan})
      : _textSpan = textSpan,
  ...
  List<TextSpan> updateSpans(
      String oldText, List<InlineSpan> oldSpans, String newText) {
      ...
      switch (diff.operation) {
        case 1: // Insert
          newSpans.add(TextSpan(
              text: diff
                  .text)); // For inserted text, create new TextSpan without style
          break;
        case 0: // Equal
          int length = diff.text!.length;
          while (length > 0) {
            if (spanIndex >= oldSpans.length) break;
            TextSpan currentSpan = oldSpans[spanIndex] as TextSpan;
            int remaining = currentSpan.toPlainText().length - spanCharIndex;
            if (remaining >= length) {
              newSpans.add(TextSpan(
                text: currentSpan
                    .toPlainText()
                    .substring(spanCharIndex, spanCharIndex + length),
                style: currentSpan.style,
              ));
              spanCharIndex += length;
              length = 0;
            } else {
              newSpans.add(TextSpan(
                text: currentSpan.toPlainText().substring(spanCharIndex),
                style: currentSpan.style,
              ));
              length -= remaining;
              spanCharIndex = 0;
  ...
  TextSpan buildTextSpan(
    ...
    for (var child in _textSpan.children ?? <TextSpan>[]) {
...
class _SelectableTextSelectionGestureDetectorBuilder
  ...
  _SelectableTextSelectionGestureDetectorBuilder({
    required _EditableSelectableTextState state,
  })  : _state = state,
        super(delegate: state);

  final _EditableSelectableTextState _state;
...
///
/// The [EditableSelectableText] widget displays a string of text with a single style.
...
/// {@end-tool}
///
/// Using the [SelectableText.rich] constructor, the [EditableSelectableText] widget can
...
/// ## Interactivity
///
/// To make [EditableSelectableText] react to touch events, use callback [onTap] to achieve
...
class EditableSelectableText extends StatefulWidget {
  ...
  const EditableSelectableText.rich(
  ...
  /// The text editor for the values.
  ...
  @override
  State<EditableSelectableText> createState() => _EditableSelectableTextState();
...
class _EditableSelectableTextState extends State<EditableSelectableText>
    implements TextSelectionGestureDetectorBuilderDelegate {
  EditableTextState? get _editableText => editableTextKey.currentState;

  late TextSpanEditingController _controller = widget.controller;
  ...
  void initState() {
    ...
    // );
    _controller.textSpan = widget.textSpan ?? TextSpan(text: widget.data);
    _controller.addListener(_onControllerChanged);
  }

  @override
  void didUpdateWidget(EditableSelectableText oldWidget) {
